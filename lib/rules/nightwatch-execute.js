...
const whitelist = [
  'Array', 'Boolean', 'Date', 'Error', 'Function', 'Number', 'Object', 'RegExp', 'String', 'Symbol',
  'window', 'document',
];
...
module.exports = {
  meta: {
    docs: {
      description: "should check undefined variables in nightwatch execute domain",
    ...
    return {
      'Program:exit': function () {
        ...
        while (stack.length) {
          ...
          const calleeName = safeGet(scope, 'block.parent.callee.property.name');
          if (apis.includes(calleeName) && isFirstArgument(scope.block.parent.arguments[0], scope.block)) {
            const undefs = scope.through
              .filter(ref => !whitelist.includes(ref.identifier.name))
              .forEach(ref => context.report({
                node: ref.identifier,

    ...
    return {
      ...
      CallExpression: function(node) {
        ...
        if (type === 'Identifier' && ['filterSelectors', 'filterDispatchers'].includes(name)) {
          ...
          const callByProps = /^(props|nextProps|prevProps)$/.test(node.callee.object.name) || /^(props|nextProps|prevProps)$/.test(getLastMemberPropertyValue(node.callee));
          if (callByProps) {
      ...
      VariableDeclarator: function(node) {
        ...
        const propsDestructuring = node.init.type === 'Identifier' && /^(props|nextProps|prevProps)$/.test(node.init.name);
      ...
      MemberExpression: function(node) {
        const isProps = /^(props|nextProps|prevProps)$/.test(node.object.name) || /^(props|nextProps|prevProps)$/.test(getLastMemberPropertyValue(node));
        if (isProps) {

...
module.exports = {
  ...
  create: function(context) {
    ...
    function verifyRuleProperty(node) {
      const keys = getKeys(node.properties);
      ['fn', 'msg', 'nMsg'].forEach((key) => {
        if (!keys.includes(key)) {
          context.report({
            message: "'{{ key }}' was required in a rule.",
            node: node,
            data: { key },
          });
        }
          ...
          if (idx !== undefined) {
            switch (expressionNode.callee.property.name) {
              case 'addAction':
                ...
                break;
              case 'addRule':
                const [ruleNameNode, ruleValue] = expressionNode.arguments;
                verifyRuleProperty(ruleValue);
                checkerProps[idx].definedRules.push({
